...
public class NavigationInteractor implements NavigationContract.Interactor {
    ...
    protected int getCount(String tableName) {
        switch (tableName.toLowerCase().trim()) {
            ...
            case org.smartregister.chw.ovc.util.Constants.TABLES.OVC_REGISTER:
                String sqlOvc =
                        "SELECT count(*) " +
                                "   from " + org.smartregister.chw.ovc.util.Constants.TABLES.OVC_REGISTER + " p " +
                                "              where p.is_closed is 0 ";
                return NavigationDao.getQueryCount(sqlOvc);
            case org.smartregister.chw.ge.util.Constants.TABLES.GE_REGISTER:
                String sqlGe =
                        "SELECT count(*) " +
